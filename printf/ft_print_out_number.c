...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   ft_print_out_number.c                              :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: frmessin <frmessin@student.42.fr>          +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/06/04 16:01:18 by frmessin          #+#    #+#             */
/*   Updated: 2022/06/04 16:01:21 by frmessin         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_print_out_nbr(t_out *tab)
{
	...
	if(tab->wdt != 0 && tab->wdt - how_big(num, 10) > 0 && tab->wdt - (tab->dot - 1) > 0 && tab->dash == 0)
	{
		if((tab->sign > 0) || sign < 0) //se c'e' il segno negativo mi serve lo spazio perche' almeno 1 e' preso dal segno di merda
			i = 1;
		while(i < tab->wdt - how_big(num,10))//scrivi finche' ne hai lo spazio - la grandezza della stringa
		{
			z+= write(1," ", 1);
			i++;
		}
	...
	if(tab->wdt != 0 && tab->wdt - how_big(num, 10) > 0 && tab->wdt - (tab->dot - 1) > 0 && tab->dash == 1)
	{
		...
		if(tab->dot -1 - how_big(num, 10) < 0)
		{
			while(i <= tab->wdt - how_big(num, 10)) // widht - num lenght buuuut. THE PRECISION!!!
			{
				z+= write(1, " ", 1);
				i++;
			}
